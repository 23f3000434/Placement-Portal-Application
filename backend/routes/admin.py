...
def approve_company(cid):
    c = CompanyProfile.query.get_or_404(cid); c.approval_status = "approved"; db.session.commit(); cache.clear()
...
def reject_company(cid):
    c = CompanyProfile.query.get_or_404(cid); c.approval_status = "rejected"; db.session.commit(); cache.clear()
...
def toggle_blacklist(cid):
    c = CompanyProfile.query.get_or_404(cid); c.is_blacklisted = not c.is_blacklisted; db.session.commit(); cache.clear()
...
def get_drives():
    ...
    if search:
        q = q.join(CompanyProfile, PlacementDrive.company_id == CompanyProfile.id).filter(
            db.or_(PlacementDrive.job_title.ilike(f"%{search}%"), CompanyProfile.company_name.ilike(f"%{search}%")))
    return jsonify([{
...
def approve_drive(did):
    d = PlacementDrive.query.get_or_404(did); d.status = "approved"; db.session.commit(); cache.clear()
    return jsonify({"message": f"'{d.job_title}' approved"}), 200

@admin_bp.route("/drives/<int:did>/reject", methods=["PUT"])
@admin_required
def reject_drive(did):
    d = PlacementDrive.query.get_or_404(did); d.status = "rejected"; db.session.commit(); cache.clear()
    return jsonify({"message": f"'{d.job_title}' rejected"}), 200

@admin_bp.route("/drives/<int:did>/close", methods=["PUT"])
@admin_required
def close_drive(did):
    d = PlacementDrive.query.get_or_404(did); d.status = "closed"; db.session.commit(); cache.clear()
...
def get_stats():
    ...
    top_companies = db.session.query(CompanyProfile.company_name, func.count(Application.id)).join(
        PlacementDrive, PlacementDrive.company_id == CompanyProfile.id).join(
        Application, Application.drive_id == PlacementDrive.id).filter(
        Application.status == "selected").group_by(CompanyProfile.company_name).order_by(
        func.count(Application.id).desc()).limit(10).all()
    branch_stats = db.session.query(StudentProfile.branch, func.count(Application.id)).join(
        Application, Application.student_id == StudentProfile.id).group_by(StudentProfile.branch).all()
    drives_monthly = db.session.query(func.strftime('%Y-%m', PlacementDrive.created_at), func.count(PlacementDrive.id)).group_by(
        func.strftime('%Y-%m', PlacementDrive.created_at)).order_by(func.strftime('%Y-%m', PlacementDrive.created_at)).all()
    return jsonify({
