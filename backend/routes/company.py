...
def dashboard():
    p = _profile()
    if not p: return jsonify({"error": "Profile not found"}), 404
...
def get_profile():
    ...
    return jsonify({"id": p.id, "company_name": p.company_name, "hr_contact": p.hr_contact,
        "website": p.website, "description": p.description,
        "approval_status": p.approval_status, "email": p.user.email}), 200
...
def update_profile():
    ...
    if d.get("company_name","").strip(): p.company_name = d["company_name"].strip()
...
def create_drive():
    ...
    db.session.add(drive); db.session.commit(); cache.clear()
...
def get_applications(did):
    ...
    if drive.company_id != p.id: return jsonify({"error": "Not your drive"}), 403
    return jsonify([{
...
def update_status(aid):
    ...
    # Companies can set any valid status including back to "applied"
    if new_status not in ("applied", "shortlisted", "selected", "rejected"):
        return jsonify({"error": "Invalid status"}), 400
    app.status = new_status; db.session.commit()
